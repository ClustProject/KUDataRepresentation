...
class PathDTWBatch(Function):
    @staticmethod
    def forward(ctx, D, gamma): # D.shape: [batch_size, N1 , N2]
        batch_size,N1,N2 = D.shape
        device = D.device
        D_cpu = D.detach().cpu().numpy()
        gamma_gpu = torch.FloatTensor([gamma]).to(device)
        
        grad_gpu = torch.zeros((batch_size, N1 ,N2)).to(device)
        Q_gpu = torch.zeros((batch_size, N1+2 ,N2+2,3)).to(device)
        E_gpu = torch.zeros((batch_size, N1+2 ,N2+2)).to(device)  
    ...
    def backward(ctx, grad_output):
        ...
        batch_size,N1,N2 = D_cpu.shape
        Hessian = torch.zeros((batch_size, N1 ,N2)).to(device)
